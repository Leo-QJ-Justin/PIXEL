...
class TestSettingsDialogInit:
    ...
    def test_has_three_sidebar_tabs(self, qtbot, tmp_path):
        dialog, _ = _make_dialog(qtbot, tmp_path)
    ...
    def test_loads_wander_chance(self, qtbot, tmp_path):
        dialog, _ = _make_dialog(qtbot, tmp_path, {"behaviors": {"wander": {"wander_chance": 0.7}}})
...
class TestSettingsEditing:
    ...
    def test_change_wander_chance(self, qtbot, tmp_path):
        ...
        slider.setValue(50)
        assert dialog._pending["behaviors"]["wander"]["wander_chance"] == 0.5
    ...
    def test_change_wander_interval_min(self, qtbot, tmp_path):
        ...
        spin.setValue(8000)
        assert dialog._pending["behaviors"]["wander"]["wander_interval_min_ms"] == 8000
    ...
    def test_change_morning_greeting(self, qtbot, tmp_path):
        dialog, _ = _make_dialog(qtbot, tmp_path)
        edit = _find_widget(dialog, QLineEdit, lambda w: w.text() == "Rise and shine!")
        edit.setText("Good morning!")
        assert (
            dialog._pending["behaviors"]["time_periods"]["greetings"]["morning"] == "Good morning!"
        )
